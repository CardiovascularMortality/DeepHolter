...
random.shuffle(live_filefolder)  # 打乱顺序
...
#in case less than 1 hour
train_bag_mask = []
test_bag_mask = []
external_valid_bag_mask = []

for folder, path_list, positional_list, bag_mask_list, cnt_name in [
    (train_cvd_filefolder, train_path, train_positional_list, train_bag_mask, "train_cvd_cnt"),
    (train_live_filefolder, train_path, train_positional_list, train_bag_mask, "train_live_cnt"),
    (test_cvd_filefolder, test_path, test_positional_list, test_bag_mask, "test_cvd_cnt"),
    (test_live_filefolder, test_path, test_positional_list, test_bag_mask, "test_live_cnt"),
    (external_valid__filefolder, external_valid_path, external_valid_positional_list, external_valid_bag_mask, "external_valid_cnt")
]:
    for i in folder:
        ...
        for hour in th["Hour"].unique():
            ...
            for j in range(tmp_len):
                ...
                bag_mask[j] = 1
        if bag_mask.count(1) == 0:
            print(f"warning: {i} has no valid bag, skipped.")
            continue
        path_list.append(tmp_list)
        positional_list.append(tmp_list2)
        bag_mask_list.append(bag_mask)
...
train_dataset = MyDataset(file_paths,y_xlsx,train_path,train_positional_list,patientinfo_xlsx,chara_paths,timeinfo_paths,cvd_file_paths,cvd_chara_paths,cvd_timeinfo_paths,bag_mask=train_bag_mask,train_mean=train_means,train_std=train_stds,scalar=scalar)
test_dataset = MyDataset(file_paths,y_xlsx,test_path,test_positional_list,patientinfo_xlsx,chara_paths,timeinfo_paths,cvd_file_paths,cvd_chara_paths,cvd_timeinfo_paths,bag_mask=test_bag_mask,train_mean=train_means,train_std=train_stds,scalar=scalar)
external_valid_dataset = MyDataset(file_paths,y_xlsx,external_valid_path,external_valid_positional_list,patientinfo_xlsx,chara_paths,timeinfo_paths,cvd_file_paths,cvd_chara_paths,cvd_timeinfo_paths,bag_mask=external_valid_bag_mask,train_mean=train_means,train_std=train_stds,scalar=scalar)
...
for epoch in range(n_epoch):  # loop over the dataset multiple times
    ...
    for inputs, positional,data_mask, patientinfo, timeinfo, chara, bag_mask, y in train_loader:
        #nan
        ...
        bag_mask = bag_mask.to(device)
        y = y.to(device)

        optimizer.zero_grad()
        # forward + backward + optimize
        pred,contra_loss,_,_,_ = model(inputs,positional,data_mask,patientinfo,timeinfo,chara,bag_mask,bc_only=False)
    ...
    #测试
    with torch.no_grad():
        for inputs, positional,data_mask, patientinfo, timeinfo, chara,bag_mask, y in test_loader:
            #判断是否存在nan
            if torch.isnan(inputs).any() :
                print("nan exists.")
            if torch.isnan(patientinfo).any() :
        ...
        for inputs, positional,data_mask, patientinfo, timeinfo, chara,bag_mask, y in external_valid_loader:
            #nan
            if torch.isnan(inputs).any() :
                print("nan exists.")
            if torch.isnan(patientinfo).any() :
